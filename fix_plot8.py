...
for i, vpath in enumerate(drowsy_videos[:20]):
    errs = process_video(vpath, max_sequences=2)
    errors_drowsy.extend(errs)
    if i % 5 == 0:
        print(f"  {i+1}/20...")

errors_alert  = np.array(errors_alert)
errors_drowsy = np.array(errors_drowsy)

print(f"\nAlert errors  — mean: {errors_alert.mean():.4f}, std: {errors_alert.std():.4f}, range: {errors_alert.min():.4f}-{errors_alert.max():.4f}")
print(f"Drowsy errors — mean: {errors_drowsy.mean():.4f}, std: {errors_drowsy.std():.4f}, range: {errors_drowsy.min():.4f}-{errors_drowsy.max():.4f}")

# clip for visibility
clip_max = np.percentile(np.concatenate([errors_alert, errors_drowsy]), 95)

fig, ax = plt.subplots(figsize=(11, 5))
ax.hist(errors_alert,  bins=40, color='#2ECC71', alpha=0.7, label='Alert sequences',  edgecolor='white')
ax.hist(errors_drowsy, bins=40, color='#E74C3C', alpha=0.7, label='Drowsy sequences', edgecolor='white')
...
ax.axvline(THRESHOLD_MODERATE, color='#C0392B', linewidth=2.5, linestyle='--', label=f'Moderate: {THRESHOLD_MODERATE:.4f}')

...
print(f"Found {len(normal_videos)} normal videos, {len(drowsy_videos)} drowsy videos")
...
for i, vpath in enumerate(normal_videos[:20]):
    ...
    if i % 5 == 0:
        print(f"  {i+1}/{min(20, len(normal_videos))} normal videos...")
...
for i, vpath in enumerate(drowsy_videos[:20]):
    ...
    if i % 5 == 0:
        print(f"  {i+1}/{min(20, len(drowsy_videos))} drowsy videos...")

print(f"\nTotal samples: {len(true_labels)} (alert: {true_labels.count('alert')}, drowsy: {true_labels.count('drowsy')})")

# ── Confusion Matrix ───────────────────────────────────────────────────────
cm = confusion_matrix(true_labels, pred_labels, labels=['alert', 'drowsy'])
report = classification_report(true_labels, pred_labels, labels=['alert', 'drowsy'])
...
plt.close()
print("\nConfusion matrix saved to plots/07_confusion_matrix.png")

# ── Error Distribution Alert vs Drowsy ────────────────────────────────────
...
plt.close()
print("Alert vs drowsy error plot saved to plots/08_alert_vs_drowsy_errors.png")
